...
function steadyframe(x::AbstractVector{<:Real},y::AbstractVector{<:Real},z::AbstractVector{<:Real},
    ...
    Q = Array{Float64}(undef,length(U),length(V))
    numer_out = Array{Float64}(undef,length(U),length(V))
    chisq = Array{Float64}(undef,length(x),length(y),length(z))
    sigsq = Array{Float64}(undef,length(x),length(y),length(z))
