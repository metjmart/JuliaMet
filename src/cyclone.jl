...
function calc_rmw{T<:Real}(r::Vector{T},z::Vector{T},azmean_vt::Array{T,2})
    ...
    rmw = Array(Float64,length(z)) .* NaN;
...
function calc_azmean{T<:Real}(r::Vector{T},z::Vector{T},var::Array{T,3})
    # Define an array for the azimuthal mean
    azmean_var = Array(Float64,length(r),length(z)) .* NaN;
