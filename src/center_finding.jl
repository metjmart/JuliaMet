...
# This older version of the function does not adjust the value for 2R80 during
# the iterative procedure. This results in ~ 3x speed-up but may not be as
...
function p_centroid(x::AbstractVector{Ta},y::AbstractVector{Tb},
    ...
    xx,yy = grid2d(x,y)
    xbar0 = Array{Float64}(undef,length(z))
    ybar0 = Array{Float64}(undef,length(z))
    pp    = similar(prs)
...
# Newer version updates the RMW, but is considerably slower...need to
# test alternative methods to speed things up

function p_centroid2(x::AbstractVector{Ta},y::AbstractVector{Tb},
                     z::AbstractVector{Tc},prs::AbstractArray{Td,3},
                     u::AbstractArray{Te,3},v::AbstractArray{Tf,3},
                     prnt_cents::Bool=false) where {Ta<:Real,Tb<:Real,Tc<:Real,Td<:Real,Te<:Real,Tf<:Real}
    ...
    for k in eachindex(z)
        if k >= z2km && k <= z8km
            ...
            while xbar0[k] != ixbar && ybar0[k] != iybar
                ...
                #println("new xbar0 = ", xbar0[k])
                #println("new ybar0 = ", ybar0[k])
                # Now we need to update the RMW at each vertical level using new center
                u_rpz = regrid_xy2rp(xbar0[k],ybar0[k],x,y,r,phi,u[:,:,k])
                v_rpz = regrid_xy2rp(xbar0[k],ybar0[k],x,y,r,phi,v[:,:,k])
                ...
                two_r80 = 2.0 * r[r80]
                #println(two_r80)
                #println("z = ",z[k]," iybar = ",iybar," ybar0 = ",ybar0[k])
            end
            #println("z = ", z[k], "Total itrs = ",itr)
        end
    end
